...
begin
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'change_basis', 'national_industry_employment',
  ...
  industry_trends := jsonb_build_object(
    ...
    'decline_leaders', coalesce((
      select jsonb_agg(to_jsonb(x) order by x.percent_change asc nulls last)
      from (
        select *
        from public.v_industry_national_signals
        where (filter_industry_slug is null or industry_slug = filter_industry_slug)
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'change_basis', 'regional_employment',
  ...
  regional_signals := jsonb_build_object(
    'items', coalesce((
      select jsonb_agg(region order by nullif(region->>'region_signal_score', '')::numeric desc nulls last, nullif(region->>'employed_latest', '')::numeric desc nulls last)
      from (
        select region
        from jsonb_array_elements(coalesce(top_region, '[]'::jsonb)) regions(region)
        order by nullif(region->>'region_signal_score', '')::numeric desc nulls last, nullif(region->>'employed_latest', '')::numeric desc nulls last
        limit case
    ...
    'growth_leaders', coalesce((
      select jsonb_agg(region order by nullif(region->>'percent_change', '')::numeric desc nulls last, nullif(region->>'absolute_change', '')::numeric desc nulls last)
      from (
        select region
        from jsonb_array_elements(coalesce(top_region, '[]'::jsonb)) regions(region)
        where nullif(region->>'percent_change', '')::numeric > 0
        order by nullif(region->>'percent_change', '')::numeric desc nulls last, nullif(region->>'absolute_change', '')::numeric desc nulls last
        limit 12
      ) growth_regions
    ), '[]'::jsonb),
    'decline_leaders', coalesce((
      select jsonb_agg(region order by nullif(region->>'percent_change', '')::numeric asc nulls last, nullif(region->>'absolute_change', '')::numeric asc nulls last)
      from (
        select region
        from jsonb_array_elements(coalesce(top_region, '[]'::jsonb)) regions(region)
        where nullif(region->>'percent_change', '')::numeric < 0
        order by nullif(region->>'percent_change', '')::numeric asc nulls last, nullif(region->>'absolute_change', '')::numeric asc nulls last
        limit 12
      ) decline_regions
    ), '[]'::jsonb),
    'source', 'Statistisk sentralbyrå',
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'absolute_change_thousands', d.absolute_change_thousands,
        'percent_change', d.percent_change,
        'context_absolute_change', d.context_absolute_change,
        'context_percent_change', d.context_percent_change,
        'change_basis', case
          when d.regional_signal is null then 'national_occupation_group'
          else 'regional_industry_field_proxy'
        end,
  ...
  from (
    ...
    regional_ranked as (
      select
        ...
        round((rol.employed_value - coalesce(rol.previous_value, rol.employed_value))::numeric, 2) as absolute_change,
        round(rol.percent_change::numeric, 2) as percent_change,
        row_number() over (
    ...
    select
      ...
      coalesce(regional.absolute_change, ms.absolute_change_thousands) as context_absolute_change,
      coalesce(regional.percent_change, ms.percent_change) as context_percent_change,
      round(
        case
  ...
  career_directions := jsonb_build_object(
    ...
    'growth_leaders', coalesce((
      select jsonb_agg(direction order by nullif(direction->>'context_percent_change', '')::numeric desc nulls last, nullif(direction->>'context_absolute_change', '')::numeric desc nulls last)
      from (
        select direction
        from jsonb_array_elements(coalesce(top_direction, '[]'::jsonb)) directions(direction)
        where nullif(direction->>'context_percent_change', '')::numeric > 0
        order by nullif(direction->>'context_percent_change', '')::numeric desc nulls last, nullif(direction->>'context_absolute_change', '')::numeric desc nulls last
        limit 12
      ) growth_directions
    ), '[]'::jsonb),
    'decline_leaders', coalesce((
      select jsonb_agg(direction order by nullif(direction->>'context_percent_change', '')::numeric asc nulls last, nullif(direction->>'context_absolute_change', '')::numeric asc nulls last)
      from (
        select direction
        from jsonb_array_elements(coalesce(top_direction, '[]'::jsonb)) directions(direction)
        where nullif(direction->>'context_percent_change', '')::numeric < 0
        order by nullif(direction->>'context_percent_change', '')::numeric asc nulls last, nullif(direction->>'context_absolute_change', '')::numeric asc nulls last
        limit 12
      ) decline_directions
    ), '[]'::jsonb),
