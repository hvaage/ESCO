...
begin
  ...
  from (
    select
      ranked.*,
      case
        when filter_industry_slug is not null and ranked.industry_slug = filter_industry_slug then 1
        when nho_region_code is not null and ranked.region_code = nho_region_code then 2
    ...
    where ranked.year = latest_nho_year
      and ranked.signal_value is not null
      and (
        filter_industry_slug is null
        or ranked.industry_slug is null
        or ranked.industry_slug = filter_industry_slug
      )
      and (
        nho_region_code is null
        or ranked.region_code is null
        or ranked.region_code = nho_region_code
      )
